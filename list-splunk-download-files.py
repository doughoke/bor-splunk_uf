...
splunk_splitter = re.compile(r'\bsplunk-\d+.\d+(?:.\d+)?-\d+-[\w.-]+.(?:tgz|rpm|deb|msi|pkg.Z|tar.Z|bin)\b')
splunk_re = re.compile(r'^splunk-(?P<version>\d+.\d+(?:.\d+)?)-(?P<build>\d+)-(?P<platform>[\w.-]+).(?P<ext>tgz|rpm|deb|msi|pkg.Z|tar.Z|bin)')
